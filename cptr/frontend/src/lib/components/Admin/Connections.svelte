<script lang="ts">
	...
	async function load() {
		...
		} catch { toast.error($t('connections.loadError')); }
		...
		{#each connections as conn}
			...
			>
				...
				<span
					...
					onkeydown={(e) => { if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); toggleEnabled(e, conn); } }}
				>
					<span class="absolute top-0.5 w-2.5 h-2.5 rounded-full transition-all duration-150
						{conn.enabled ? 'left-3 bg-white dark:bg-gray-900' : 'left-0.5 bg-white dark:bg-gray-500'}"></span>
				</span>
			</button>
		{/each}
...
{#if editConn}
	<EditConnectionModal connection={editConn} onclose={() => editConn = null} onchanged={handleChanged} />
{/if}
