...
{:else}
	<div>
		{#each pagedUsers as user}
			...
			>
				<img src={user.profile_image_url || '/user.png'} alt="" class="w-4 h-4 rounded-full object-cover shrink-0" />
				<span class="flex-1 text-[13px] text-gray-700 dark:text-gray-300 truncate">{user.display_name || user.username}</span>
				<span class="text-[11px] transition-colors duration-75
					{user.role === 'admin' ? 'text-gray-900 dark:text-white font-medium'
	...
	{#if totalPages > 1}
		<div class="flex items-center justify-between mt-4 pt-3 border-t border-gray-200 dark:border-white/6">
			<button
...
{#if editUser}
	<EditUserModal user={editUser} {adminCount} currentUserId={$session?.user_id ?? ''} onclose={() => editUser = null} onchanged={handleChanged} />
{/if}
