<script lang="ts">
	...
	async function create() {
		if (!username.trim()) { toast.error($t('admin.usernameRequired')); return; }
		if (password.length < 6) { toast.error($t('admin.minCharsPassword')); return; }
		creating = true;
		try {
...
<Modal {onclose} class="w-full max-w-sm mx-4">
	...
	<div class="p-4" onkeydown={(e) => { if (e.key === 'Enter' && username.trim() && password.length >= 6) create(); }}>
		...
		<input type="text" placeholder={$t('admin.enterUsername')} bind:value={username} autofocus autocomplete="off" spellcheck="false"
		...
		<label class="text-[10px] text-gray-400 dark:text-gray-600 mt-1">{$t('admin.displayName')}</label>
		<input type="text" placeholder={$t('admin.optional')} bind:value={displayName} autocomplete="off" spellcheck="false"
			class="block w-full bg-transparent text-[13px] text-gray-700 dark:text-gray-300 placeholder:text-gray-300 dark:placeholder:text-gray-700 outline-none py-0.5" />
		<label class="text-[10px] text-gray-400 dark:text-gray-600 mt-1">{$t('admin.passwordLabel')}</label>
		<input type="password" placeholder={$t('admin.minChars')} bind:value={password} autocomplete="new-password"
			class="block w-full bg-transparent text-[13px] text-gray-700 dark:text-gray-300 placeholder:text-gray-300 dark:placeholder:text-gray-700 outline-none py-0.5" />
		...
		<div class="flex justify-end mt-2">
			<button disabled={creating || !username.trim() || password.length < 6} onclick={create}
