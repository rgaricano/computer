<script lang="ts">
	...
	import { updatePassword as apiUpdatePassword, uploadAvatar, deleteAvatar, updateProfile } from '$lib/apis/auth';
	...
	async function updatePassword() {
		if (!currentPassword || !newPassword) { toast.error($t('account.bothFieldsRequired')); return; }
		if (newPassword.length < 6) { toast.error($t('auth.minChars')); return; }
		...
		finally { updatingPassword = false; }
	}
...
<div class="flex items-center gap-4 mb-6">
	...
	>
		...
		{:else}
			...
			<div class="absolute inset-0 bg-black/0 group-hover:bg-black/40 transition-colors duration-200 flex items-center justify-center">
				<svg class="w-4 h-4 text-white opacity-0 group-hover:opacity-100 transition-opacity duration-200 drop-shadow-sm" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5">
	...
	<div class="flex flex-col gap-1">
		<span class="text-sm font-medium text-gray-900 dark:text-white">{$session?.display_name || $session?.username}</span>
		<div class="flex items-center gap-2">
			<button
				class="text-[11px] text-gray-400 hover:text-gray-600 dark:hover:text-gray-300 transition-colors duration-100"
				onclick={handleAvatarSelect}
				disabled={uploadingAvatar}
			>{$t('account.uploadPhoto')}</button>
			{#if avatarUrl}
